...
//   - onBack: () => void, 返回按钮.
const PipelineRunDetailView = ({ detail, loading, title, onReload, onCancel, onReplay, onApprove, onBack }) => {
  ...
  const renderLogs = step => {
    ...
    if (!text) return <pre className="build-log">暂无日志</pre>;
  ...
  const cancelable = onCancel && detail?.pipeline && isPipelineStatusActive(detail.pipeline.status);
  // replay 仅在 run 已结束时启用, 与 取消构建 互斥 (cancel 只在 active 时显示).
  const replayable = onReplay && detail?.pipeline && !isPipelineStatusActive(detail.pipeline.status);
  ...
  return (
    <div className="ops-build-detail">
            ...
            onClick={async () => {
              ...
              try {
                await onReplay({
                  ...
                  variables: detail.pipeline.additional_variables || {}
                });
              } catch (err) {
        ...
        {cancelable && (
          <Button
            ...
            onClick={async () => {
              ...
              setCanceling(true);
              try {
                await onCancel(detail.pipeline.id);
      ...
      <Card title={title || `构建详情 · #${detail?.pipeline?.number || ''}`}>
        {loading && !detail ? (
