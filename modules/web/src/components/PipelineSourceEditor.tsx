...
const emptyVarRow = (): VarRow => ({ key: '', value: '' });

const variablesToRows = (vars: unknown): VarRow[] => {
...
const rowsToVariables = (rows: VarRow[]) =>
  (rows || [])
...
const PipelineSourceEditor = ({ value, onChange, disabled = false, repoId }) => {
  ...
  const [preview, setPreview] = useState<{
    ...
    loading: boolean;
    error: string;
  }>({ content: '', missing: [], loading: false, error: '' });
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      const items = itemsFromListResponse(all);
      setAllTemplates(items);
      setStepTemplates(items.filter(t => (t as { kind?: string }).kind === 'step'));
    } catch (err) {
      message.error((err as Error)?.message || '加载模板列表失败');
    } finally {
  ...
  const pipelineKindTemplates = useMemo(
    () =>
      allTemplates.filter((t: unknown) => {
        const o = t as { kind?: string };
        return !o.kind || o.kind === 'pipeline';
      }),
    [allTemplates]
  );
  ...
  const refreshPreview = useCallback(
    async (overrideVars?: Record<string, unknown>) => {
      if (!templateId) {
      ...
      try {
        const data = (await renderTemplate(templateId, overrideVars ?? variables, repoId)) as {
          content?: string;
          missing?: unknown[];
        };
        setPreview({
        ...
        setPreview({
          ...
          missing: [],
          loading: false,
          error: (err as Error)?.message || '加载预览失败'
        });
      }
    },
  ...
  const updateVarRow = (idx: number, field: 'key' | 'value', fieldValue: string) => {
    setVarRows(prev => {
