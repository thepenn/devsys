...
const META = {
  [PIPELINE_STATUS.SUCCESS]: { label: '成功', className: 'success', bulletClass: 'success' },
  [PIPELINE_STATUS.FAILURE]: { label: '失败', className: 'failure', bulletClass: 'failure' },
  [PIPELINE_STATUS.ERROR]: { label: '出错', className: 'failure', bulletClass: 'failure' },
  [PIPELINE_STATUS.KILLED]: { label: '终止', className: 'killed', bulletClass: 'failure' },
  [PIPELINE_STATUS.CANCELED]: { label: '已取消', className: 'killed', bulletClass: 'failure' },
  [PIPELINE_STATUS.RUNNING]: { label: '运行中', className: 'running', bulletClass: 'running', cancellable: true, active: true },
  [PIPELINE_STATUS.PENDING]: { label: '等待', className: 'pending', bulletClass: 'pending', cancellable: true, active: true },
  [PIPELINE_STATUS.BLOCKED]: { label: '等待审批', className: 'pending', bulletClass: 'pending', cancellable: true, active: true },
  [PIPELINE_STATUS.SKIPPED]: { label: '跳过', className: 'pending', bulletClass: 'pending' },
  [PIPELINE_STATUS.NOT_RUN]: { label: '未执行', className: 'not-run', bulletClass: 'not-run', bulletEmpty: true },
  [PIPELINE_STATUS.UNKNOWN]: { label: '未知', className: 'pending', bulletClass: 'pending' }
};
...
export function getPipelineStatusMeta(value) {
  ...
  return META[normalized] || META[PIPELINE_STATUS.UNKNOWN];
}
...
export function formatApprovalState(value) {
  const normalized = String(value || '').trim().toLowerCase();
  return (APPROVAL_STATE_META[normalized] || APPROVAL_STATE_META[APPROVAL_STATES.PENDING]).label;
}
