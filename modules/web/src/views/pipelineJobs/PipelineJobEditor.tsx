...
const emptyVar = (): VarRow => ({ key: '', value: '' });
...
const PipelineJobEditor = () => {
  ...
  const fetchJob = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = (await getJob(numericId)) as Record<string, any>;
      setJob(data);
  ...
  useEffect(() => {
    ...
    listCertificates({ per_page: 200 })
      .then(data => setCerts(itemsFromListResponse(data)))
      .catch(err => {
        // 凭证拉不到不影响其它功能
  ...
  const fetchRuns = useCallback(
    async (targetPage = 1) => {
      ...
      setRunsLoading(true);
      try {
        const data = (await listJobRuns(numericId, { page: targetPage, per_page: runsPerPage })) as {
          total?: number;
          page?: number;
        };
        setRuns(itemsFromListResponse(data));
