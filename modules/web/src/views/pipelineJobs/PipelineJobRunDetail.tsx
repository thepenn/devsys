...
const PipelineJobRunDetail = () => {
  ...
  const livePoll = useMemo(
    () => ({
      fetchMeta: () => getJobRunMeta(Number(id), Number(runId)),
      fetchLogs: (stepId, afterLine) =>
        getJobRunStepLogs(Number(id), Number(runId), stepId, { after_line: afterLine, limit: 1000 }),
      ...(detail?.pipeline != null
        ? {
            buildStreamUrl: (stepId: number) =>
              `/pipeline-jobs/${id}/runs/${runId}/steps/${stepId}/stream/logs`
          }
        : {})
    }),
    [id, runId, detail?.pipeline]
  );
  ...
  return (
    <PipelineRunDetailView
      ...
      livePoll={id && runId ? livePoll : undefined}
      onReload={loadDetail}
      ...
      onReplay={async params => {
        ...
        const result = (await triggerJob(Number(id), { variables: params.variables || {} })) as { id?: number };
