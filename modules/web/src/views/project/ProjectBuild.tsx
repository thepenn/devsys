...
const ProjectBuild = () => {
  ...
  const fetchRuns = useCallback(
    async (targetPage = 1) => {
      ...
      setLoadingRuns(true);
      try {
        const data = (await listPipelineRuns(selectedRepo.id, {
          page: targetPage,
          per_page: perPage
  ...
  const openRunModal = () => {
    ...
    const lastCommit =
      runs.length > 0 && typeof runs[0]?.commit === 'string' ? String(runs[0].commit).trim() : '';
    setRunForm({
  ...
  const buildSettingsPayload = useCallback(
    (overrides: Record<string, unknown> = {}) => ({
      ...
      cron_schedules: (Array.isArray(overrides.cron_schedules)
        ? (overrides.cron_schedules as string[])
        : cronRows
      )
        ...
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <label className="modal-field">
            ...
            <Input
              ...
              placeholder="必填：完整 SHA；默认可用最近一次构建的提交"
            />
          </label>
